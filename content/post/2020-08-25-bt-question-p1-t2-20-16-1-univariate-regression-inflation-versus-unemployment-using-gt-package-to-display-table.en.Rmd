...
BT is already known for our practice questions, but I wanted to take it a step further. I've been writing (painting?) a fresh coat of questions on the regression topic. It takes more time, but I've been pulling actual datasets. When combined with actual code-based regression, the questions sets are much more realistic than your average academic fare. Let's face it: the quantitative topics (e.g., regression) are all done in software, not calculators, so in a way, it's our responsibility to help candidates get practice in realistic toolsets. 

This first question in the new regression set reads:
